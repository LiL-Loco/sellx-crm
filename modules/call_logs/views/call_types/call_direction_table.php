...
foreach ($rResult as $aRow) {
    ...
    //if($aRow['is_default']){
        ...
        //$row[] = $options;
    //}else {
        $options = icon_btn('call_logs/call_direction/' . $aRow['id'], 'fa fa-pen-to-square', 'btn-default call_btn', [
            'onclick' => 'edit_call_direction(this,' . $aRow['id'] . '); return false', 'data-name' => $aRow['name'],
        ]);
        $row[] = $options .= icon_btn('call_logs/delete_call_direction/' . $aRow['id'], 'fa fa-trash-can', 'btn-danger _delete call_btn call_btn_del');
    //}
    $output['aaData'][] = $row;
}
