...
class Customer_service_model extends App_Model
{
	...
	public function add_mail_scan_rule($data)
	{
		...
		$data['status'] = 'enabled';
		$data['datecreated'] = date('m-d-Y H:i:s');
	...
	public function update_category($data, $id)
	{
		...
		$data['dateupdated'] = date("m-d-Y H:i:s");
			...
			if (!$department_id) {
				...
				if ($to == $email) {
					...
					if ($result) {
						...
						if($userid) {
							$filterdate = date('m-d-Y H:i:s', strtotime('-15 minutes'));
										...
										if ($tid) {
											...
											if($category){
												...
												if($workflow){
													...
													$ticket_workflows['datecreated'] = date('m-d-Y H:i:s');
	...
	public function update_ticket($data, $id)
	{
		...
		$data['dateupdated'] =  date('m-d-Y H:i:s');
		$data['last_update_time'] =  date('m-d-Y H:i:s');
		$this->db->where('id', $id);
		...
		if($type == 'priority'){
			...
			if($status == 'closed'){
				...
				if($get_ticket){
					...
					$time_spent = strtotime(date('m-d-Y H:i:s')) - strtotime($datecreated);
					$time_spent = $time_spent/3600;
				}
				$this->db->where('id', $id);
				$this->db->update(db_prefix() . 'cs_tickets', ['status' => $status, 'time_spent' => $time_spent, 'last_update_time' => date('m-d-Y H:i:s')]);
			}else{
				$this->db->where('id', $id);
	...
	public function add_ticket_reply($data)
	{
		...
		if(isset($data['ticket_status'])){
			...
			$this->db->update(db_prefix().'cs_tickets', ['status' => 'closed', 'last_update_time' => date('m-d-Y H:i:s'), 'dateupdated' => date('m-d-Y H:i:s'), 'time_spent' => $time_spent]);
		}
		if(isset($data['resolution'])){
			...
			$this->db->where('id', $data['ticket_id']);
			$this->db->update(db_prefix().'cs_tickets', ['resolution' => $data['response'], 'last_update_time' => date('m-d-Y H:i:s'), 'dateupdated' => date('m-d-Y H:i:s')]);

		}
		$data_reply['datecreated'] = date('m-d-Y H:i:s');
	...
	public function add_ticket_internal_reply($data)
	{
		...
		if(isset($data['cs_ticket_status']) && new_strlen($data['cs_ticket_status']) > 0){
			...
			$this->db->where('id', $data['ticket_id']);
			$this->db->update(db_prefix().'cs_tickets', ['status' => $data['cs_ticket_status'], 'last_update_time' => date('m-d-Y H:i:s'), 'dateupdated' => date('m-d-Y H:i:s'), 'time_spent' => $time_spent]);
		}
		if(isset($data['internal_resolution'])){
			...
			/*update ticket resolution*/
			$this->db->where('id', $data['ticket_id']);
			$this->db->update(db_prefix().'cs_tickets', ['resolution' => $data['note_details'], 'last_update_time' => date('m-d-Y H:i:s'), 'dateupdated' => date('m-d-Y H:i:s')]);
	...
	public function add_department_transfer($data)
	{
		...
		if ($insert_id) {
			...
			if(is_numeric($data['department_id'])){
				/*update ticket resolution*/
				$this->db->where('id', $data['ticket_id']);
				$this->db->update(db_prefix().'cs_tickets', ['department_id' => $data['department_id'], 'last_update_time' => date('m-d-Y H:i:s'), 'dateupdated' => date('m-d-Y H:i:s')]);
			}
	...
	public function add_ticket_reassign($data)
	{
		...
		if ($insert_id) {
			...
			if(is_numeric($data['assignee_id'])){
				/*update ticket resolution*/
				$this->db->where('id', $data['ticket_id']);
				$this->db->update(db_prefix().'cs_tickets', ['assigned_id' => $data['assignee_id'], 'last_update_time' => date('m-d-Y H:i:s'), 'dateupdated' => date('m-d-Y H:i:s')]);
			}
	...
	public function cs_add_email_template($data)
	{
		...
		$slug = slug_it($subject).'-'.strtotime(date('m-d-Y H:i:s'));
			...
			if(count($pre_node) > 0){
				foreach ($pre_node as $value) {
					...
					if($logs){
						...
						if($wait_date <= strtotime(date('m-d-Y H:i:s'))){
							return true;
						}
					}
				}
		...
		if($sla){
			if($sla->hours_of_operation == 'business_hours'){
				...
				if(new_strlen($cs_business_days) > 0){
					$hours = (strtotime(date('m-d-Y').' '.$cs_business_to_hours.':00') - strtotime(date('m-d-Y').' '.$cs_business_from_hours.':00'))/60/60;
					if($hours > 0){
						$due_data = $this->get_due_date((int)$sla->grace_period, $hours);

					}else{
						$due_data = date('m-d-Y H:i:s', strtotime($due_data."+ ".(int)$sla->grace_period." hours"));
					}
	...
	public function get_due_date($grace_period, $business_hours) {
		...
		do {
			...
			if(in_array($business_setting, $arr_business_days)){
				...
				$start_business_hours = strtotime(date("m-d-Y", strtotime($current_time)).' '.$cs_business_from_hours.':00');
				$end_business_hours = strtotime(date("m-d-Y", strtotime($current_time)).' '.$cs_business_to_hours.':00');
				...
				if(strtotime($due_date) < $start_business_hours){
					...
					if($grace_period_minutes < $new_business_minutes){
						$due_date = date("m-d-Y H:i:s",strtotime(date("m-d-Y", strtotime($current_time)).' '.$cs_business_from_hours.':00'."+ ".(int)$grace_period_minutes." minutes"));

						$grace_period_minutes = 0;
					}else{
						$due_date = date("m-d-Y H:i:s",strtotime(date("m-d-Y", strtotime($current_time)).' '.$cs_business_from_hours.':00'."+ ".$new_business_minutes." minutes"));
					...
					if($grace_period_minutes < $new_business_minutes){
						$due_date = date("m-d-Y H:i:s",strtotime(date("m-d-Y H:i:s", strtotime($current_time))."+ ".(int)$grace_period_minutes." minutes"));


						$grace_period_minutes = 0;
					}else{
						$due_date = date("m-d-Y H:i:s",strtotime(date("m-d-Y H:i:s", strtotime($current_time))."+ ".(int)$new_business_minutes." minutes"));
						$grace_period_minutes = (float)$grace_period_minutes - (float)$new_business_minutes;
					}
				}
			}
			$current_time = date("m-d-Y H:i:s", strtotime(date("m-d-Y",strtotime($current_time)).' '.$cs_business_from_hours.':00'."+ 1 days"));
	...
	public function update_ticket_first_reply_time($ticket_id)
	{
		...
		if($ticket){
			if($ticket->first_reply_time == null){
				$this->db->where('id', $ticket_id);
				$ticket = $this->db->update(db_prefix() . 'cs_tickets', ['first_reply_time' => date('m-d-Y H:i:s')]);
			}

			$this->db->where('id', $ticket_id);
			$ticket = $this->db->update(db_prefix() . 'cs_tickets', ['last_response_time' => date('m-d-Y H:i:s')]);
		}
		return true;
	}
	...
	public function cs_get_list_item_warranty_by_invoice($invoice_id = '', $item_id = '')
	{
		...
		if(cs_get_status_modules('service_management')){
			$sql_where = db_prefix().'sm_service_details.invoice_id = '.$invoice_id.' AND date_format('.db_prefix().'sm_service_details.expiration_date, "%Y-%m-%d") >= "'.date('m-d-Y').'"';
